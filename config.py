...
COMMON_CONTROL_IDS = [1, 2, 3, 4, 8, 10, 14, 15, 17, 20, 21, 23, 24, 26]
COMMON_PATIENT_IDS = [1, 2, 3, 4, 5, 6, 7, 10, 11, 12, 16, 17]
...
def get_common_participant_ids(group, windows=((1, 4), (4, 7))):
    ...
    if group == "ctrl":
        return list(COMMON_CONTROL_IDS)
    if group == "patient":
        return list(COMMON_PATIENT_IDS)
    return []
...
def get_common_participant_ids_by_group(windows=((1, 4), (4, 7))):
    del windows  # kept for backward-compatible signature
    return {
        "ctrl": list(COMMON_CONTROL_IDS),
        "patient": list(COMMON_PATIENT_IDS),
    }
